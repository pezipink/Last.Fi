...
type Runtime private() = 
  static member Run(?expression, ?port, ?directory, ?components, ?browse) =
    ...
    let source = FunScript.Compiler.Compiler.Compile(main, components=components).Replace("http://lastfi","http://" + ipAddress)
    ...
    //let url = sprintf "http://192.168.137.37:%d/" port
    let url = sprintf "http://%s:%d/" (getIpAddress().ToString()) port
